# Labs - CTeSP Programação de Sistemas de Informação (UA/ESTGA)

Soluções dos laboratórios de Programação em Java e futuramente outras linguagens, **Universidade de Aveiro - ESTGA** (2025/2026).

📚 Curso: CTeSP Programação em Sistemas de Informação  
👨‍🎓 Student: Gonçalo Monteiro  
🔗 LinkedIn: [gncalomonteiro](https://linkedin.com/in/goncalomonteiro-dev)  
📧 Email: gds.monteiroo@gmail.com

## 📋 Índice de Exercícios

| Folha | Exercícios | Descrição |
|-------|------------|-----------|
| [Folha 1](src/folha1/) | 7 exercícios | Loops, condicionais, arrays, geometria básica |
| [Folha 2](src/folha2/) | ... | 


## 📊 Skills Demonstradas

- Estruturas de controlo (if/switch, loops while/do-while)
- Arrays e funções auxiliares
- Validações de input (ex: números positivos, divisão por zero)
- Boas práticas: comentários, Scanner.close(), funções reutilizáveis

## 📄 Fontes

- [Folha de Exercícios 1 (PDF)](Folhas-Exercicios/Folha1.pdf)
- Resolvido com explicações inline nos códigos.

⭐ **Star se ajudou!** Feedback em Issues. Próximas folhas a caminho...

---

*Built during night classes @ UA while balancing day job. #Java #ProgrammingLabs #CTeSP*
